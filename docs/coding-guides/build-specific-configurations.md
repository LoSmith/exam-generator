...
The ngx-rocket `env` task.ts solves this problem really well, and avoids the need for separate `environment.ts` files for
...
2. add that variable name to the npm script's `env` task.ts

Now, as long as you have that environment variable set in the shell running the build, the `env` task.ts will save it into
...
variables you pass to the env task.ts! As usual, you should **never add sensitive keys or secrets to the env task.ts**.
